...
class TestFindMlxWhisper:
    ...
    def test_found_in_search_paths(self, tmp_path, monkeypatch):
        ...
        with patch("kiro_crew.transcribe.shutil.which", return_value=None):
            monkeypatch.setattr("kiro_crew.transcribe._python3_bin_dir", lambda: "")
            monkeypatch.setattr("kiro_crew.transcribe._MLX_WHISPER_SEARCH_PATHS", [str(binary)])
...
class TestFindParakeetMlx:
    def test_found_on_path(self):
        with patch("kiro_crew.transcribe.shutil.which", return_value="/usr/local/bin/parakeet-mlx"):
            assert _find_parakeet_mlx() == "/usr/local/bin/parakeet-mlx"

    def test_not_found(self, monkeypatch):
        with patch("kiro_crew.transcribe.shutil.which", return_value=None):
            monkeypatch.setattr("kiro_crew.transcribe._PARAKEET_MLX_SEARCH_PATHS", ["/nonexistent"])
    ...
    def test_found_in_search_paths(self, tmp_path, monkeypatch):
        ...
        binary.chmod(0o755)
        with patch("kiro_crew.transcribe.shutil.which", return_value=None):
            monkeypatch.setattr("kiro_crew.transcribe._PARAKEET_MLX_SEARCH_PATHS", [str(binary)])
...
class TestIsAvailable:
    ...
    def test_parakeet_available_when_binary_found(self):
        ...
        with patch("kiro_crew.transcribe._find_parakeet_mlx", return_value="/usr/bin/parakeet-mlx"):
            assert is_available(cfg) is True
...
class TestTranscribeAudio:
    ...
    async def test_whisper_discovery_runs_off_event_loop(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kiro_crew.transcribe._python3_bin_dir", discover_python_bin_dir)
    ...
    @pytest.mark.asyncio
    async def test_whisper_output_file_io_runs_off_event_loop(self, tmp_path, monkeypatch):
        from threading import get_ident
        ...
        monkeypatch.setattr("kiro_crew.transcribe._collect_whisper_output", collect_output)
    ...
    async def test_successful_transcription(self, tmp_path):
        ...
        with patch("kiro_crew.transcribe.asyncio.create_subprocess_exec", side_effect=fake_exec):
            result = await transcribe_audio(str(audio), cfg)
        assert result == "Hello world"
    ...
    async def test_whisper_failure_returns_none(self, tmp_path):
        ...
        with patch("kiro_crew.transcribe.asyncio.create_subprocess_exec", return_value=mock_proc):
            result = await transcribe_audio(str(audio), cfg)
        assert result is None
    ...
    async def test_timeout_returns_none(self, tmp_path):
        ...
        with patch("kiro_crew.transcribe.asyncio.create_subprocess_exec", return_value=mock_proc):
            with patch("kiro_crew.transcribe.asyncio.wait_for", side_effect=asyncio.TimeoutError):
                result = await transcribe_audio(str(audio), cfg)
        assert result is None
    ...
    async def test_mlx_invalid_model_rejected_before_subprocess(self, tmp_path):
        ...
        audio = tmp_path / "test.webm"
        audio.write_text("fake audio")
        cfg = SttConfig(enabled=True, provider="mlx", mlx_model="; rm -rf ~", timeout_secs=10)
        with patch("kiro_crew.transcribe._find_mlx_whisper", return_value="/usr/bin/mlx_whisper"):
...
class TestFfmpegEnsuredForWhisper:
    ...
    async def test_ensure_ffmpeg_not_called_for_transcribe(self, tmp_path):
        ...
        with (
            ...
            patch("kiro_crew.transcribe.ensure_ffmpeg_in_path") as mock_ensure,
            patch(
                "kiro_crew.transcribe._transcribe_aws", new_callable=AsyncMock, return_value="hi"
            ),
        ):
            await transcribe_audio(str(audio), cfg)
...
class TestRunWhisperCliTempOwnership:
    ...
    async def test_cancellation_reaps_the_child_before_removing_out_dir(
        ...
        with patch("asyncio.create_subprocess_exec", return_value=_Proc()):
            with pytest.raises(asyncio.CancelledError):
                await transcribe._run_whisper_cli("/fake/whisper", lambda d: [d], 10, label="test")
    ...
    @pytest.mark.asyncio
    async def test_success_still_hands_the_transcript_to_the_caller(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_cancellation_during_spawn_still_removes_out_dir(self, tmp_path, monkeypatch):
        """A cancellation landing on ``create_subprocess_exec`` itself means no
