...
class TestSttEnabledHasOneDefault:
    """`stt.enabled` resolves to the dataclass default whether or not the key is written.

    Asserted against ``SttConfig().enabled`` rather than a literal, so the pin is the
    AGREEMENT between the two paths: a later deliberate flip of the default still passes,
    while re-splitting them (a loader fallback that differs from the field default) fails.
    That split is invisible otherwise — the schema, the shipped configuration docs and
    ``kirocrew doctor`` all read the field default, so only a load of a config.json that
    omits the key can observe the loader disagreeing with them.
    """

    def test_no_config_file_uses_the_field_default(self, tmp_path: Path) -> None:
        with unittest.mock.patch("kiro_crew.config.loader.config_dir", return_value=tmp_path):
            cfg = KiroCrewConfig.load()
        assert cfg.stt.enabled is SttConfig().enabled

    def test_config_file_without_an_stt_block_uses_the_field_default(self, tmp_path: Path) -> None:
        (tmp_path / "config.json").write_text(json.dumps({}))
        with unittest.mock.patch("kiro_crew.config.loader.config_dir", return_value=tmp_path):
            cfg = KiroCrewConfig.load()
        assert cfg.stt.enabled is SttConfig().enabled

    def test_stt_block_omitting_enabled_uses_the_field_default(self, tmp_path: Path) -> None:
        (tmp_path / "config.json").write_text(json.dumps({"stt": {"provider": "whisper"}}))
        with unittest.mock.patch("kiro_crew.config.loader.config_dir", return_value=tmp_path):
            cfg = KiroCrewConfig.load()
        assert cfg.stt.enabled is SttConfig().enabled

    def test_an_explicit_false_is_still_honoured(self, tmp_path: Path) -> None:
        (tmp_path / "config.json").write_text(json.dumps({"stt": {"enabled": False}}))
        with unittest.mock.patch("kiro_crew.config.loader.config_dir", return_value=tmp_path):
            cfg = KiroCrewConfig.load()
