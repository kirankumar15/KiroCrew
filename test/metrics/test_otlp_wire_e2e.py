...
def test_collector_script_download_stays_checksum_pinned():
    """The opt-in tier downloads and EXECUTES a binary; that stays verified.

    This is not an existence check with extra steps. The two assertions below are
    the properties that make running a downloaded binary defensible at all -- a
    digest to compare against and a pinned version to fetch -- and neither is
    enforced by anything else, since the script is deliberately not a test and CI
    never runs it. A script the docs reference but that does not exist is a
    separate, lesser problem, covered by the path assertion.
    """
