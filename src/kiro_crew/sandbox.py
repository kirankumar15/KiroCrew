...
#: The governance-keystone leaves (``security._CREW_SECRET_LEAVES``), spelled here so
#: this module needs no import from ``security``. Pinned equal to that source-of-truth
#: by ``test_governance_distribution`` so the launcher list and the security floor
#: cannot drift. Used both as ``$HOME``-relative entries in the dir lists below and,
#: joined with the RESOLVED data home, by ``_relocated_keystone_dirs`` so a relocated
#: ``KIROCREW_HOME`` does not carry the ceiling out from under the launcher mask.
...
_STRICT_DIRS: list[str] = [
    ...
    # ceilings, the computer-use enable, the per-surface ``profiles`` dir, and the
    # SEL/token signing keys. Listed as explicit literals rather than sourced from
    # ``sensitive_home_dirs()`` at runtime: the child classifies each entry with its own
...
_STANDARD_DIRS: list[str] = [
    ...
    # opt-out ceilings, computer-use enable, ``profiles`` dir and signing keys are
    # bind-mount-hidden here too.
    ".kiro/crew/security_policy.json",
    ".kirocrew/security_policy.json",
    ".kiro/crew/admission_policy.json",
    ".kirocrew/admission_policy.json",
...
def _relocated_keystone_dirs() -> list[str]:
    ...
    seven ceiling leaves in ``_KEYSTONE_LEAVES``. The dir-list entries are ``$HOME``-relative
    ...
    signing keys on a relocated home. The keystone must not inherit that limitation: the
    argument the cache makes for itself holds at least as strongly here, since these ARE the
    ceiling documents and the SEL/token signing keys, and ``security._sensitive_launch_targets``
    already anchors the same leaves against the resolved home for the in-process gate — the
    OS-sandbox mask has to match so the two mechanisms do not diverge on a relocated home.
...
def _is_keystone_leaf(path: str) -> bool:
    ...
    call on the spawn path, mirroring :func:`_is_policy_cache_dir`. The leaf names
    (``security_policy.json``, ``profiles``, the signing keys, …) are specific enough not
    to collide with the credential dirs (``.aws``, ``.gnupg``, …) the seatbelt profile also
    denies, so the macOS write deny keyed on this stays scoped to the keystone.
    """
