...
def _doctor_credentials(issues: list[str]) -> None:
    ...
    if not has_config and not has_creds:
        ...
        _print_wrapped(
            ...
            "never needs to read the files. Run `aws configure sso` or `aws configure` "
    ...
    if has_config or has_creds:
        ...
        _print_wrapped(
            ...
            "credential-file block rather than a missing setup — see the "
            "blocked-commands page in the dashboard's Docs, or "
            "src/kiro_crew/docs/blocked-commands.md."
        )
    else:
        _print_wrapped(
