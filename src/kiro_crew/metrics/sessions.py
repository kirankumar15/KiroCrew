...
async def record_session_started(session_key: str) -> None:
    ...
    **Why this is a coroutine, and awaited under the caller's lock.** Two
    ...
    if _crumbs_enabled():
        try:
            await asyncio.to_thread(_write_crumb, session_key, started_at)
        except Exception:
...
def discard_session_start(session_key: str) -> None:
    ...
    with _live_lock:
        _live_starts.pop(session_key, None)
...
def _write_crumb(session_key: str, started_at: float) -> None:
    ...
    **The post-write unlink is generation-MATCHED, and that is the whole
    difference from an earlier attempt.** Unlinking the path unconditionally is
    what made this unsafe before: a successor registering under the same key has
    its own crumb at the same path, and a late predecessor would delete it. Reading
    the crumb back and removing it only when it still holds THIS ``started_at``
    means a predecessor can only ever clean up after itself.
    ...
    try:
        ...
        with _crumb_io_lock:
            ...
            if not _is_current_generation(session_key, started_at):
                if _read_crumb(path)[1] == started_at:
                    _unlink(path)
    except Exception:
...
def record_session_ended(session_key: str, *, end_reason: str) -> None:
    ...
    **The crumb unlink runs INLINE, and outside every lock.** It is one ``unlink``
    ...
    # Unlinked OUTSIDE the lock, and whether or not a duration was emitted: the
    # session is over, so its crumb must not survive for the next boot to read as
    # a crash. No lock is needed and none may be taken -- this runs on the event
    # loop, and the only other toucher of this path is the backfill, which skips
    # every crumb whose owner is live and therefore never looks at one this
    # process wrote.
    _unlink(crumb)
    ...
    *started_before* is an epoch cutoff: a crumb whose ``started_at`` is at or
    ...
    try:
        ...
        for path in sorted(crumb_dir.glob("*.json")):
            # Read, judge and unlink under ONE lock hold. Split apart, a same-key
            # session registering between the read and the unlink has its fresh
            # crumb deleted here, and its later crash then has no record at all.
            # The cutoff alone does not cover it: this decides on the OLD crumb's
            # started_at, so a pre-cutoff read authorises unlinking a path that by
            # then holds a post-cutoff successor.
            with _crumb_io_lock:
