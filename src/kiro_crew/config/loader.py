...
class SttConfig:
    ...
    Enabled by default: the surface is offered, but recognition runs through a
    whisper CLI the user installs out of band and a system ffmpeg, so a host
    missing either reports it from ``kirocrew doctor`` rather than by having the
...
class KiroCrewConfig:
    ...
    def _load_resolved(cls) -> tuple[KiroCrewConfig, int]:
        ...
        cfg = cls(
            ...
            stt=SttConfig(
                # Must match SttConfig.enabled's field default. A disagreement gives
                # the one field two defaults depending on whether a config.json
                # exists: an install without one resolves the dataclass default,
                # while an install whose config.json omits the key resolves this
                # fallback, so a mismatch silently withdraws voice input from every
                # config that never named the key.
