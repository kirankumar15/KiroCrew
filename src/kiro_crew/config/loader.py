...
def _tailscale_config_from(raw: object, degraded: set[str] | None = None) -> TailscaleConfig:
    ...
    * An unrecognised ``pin_scope`` falls back to ``"node"`` (the narrower
    ...
    if data.get("trust_identity") is not None and not isinstance(data.get("trust_identity"), bool):
    ...
    _flag_raw = data.get("trust_identity")
    _allowlist_is_live = trust_identity or (
        _flag_raw is not None and not isinstance(_flag_raw, bool)
    )
    if _allowlist_is_live and raw_logins is not None and not isinstance(raw_logins, list):
    ...
    def _load_resolved(cls) -> tuple[KiroCrewConfig, int]:
        ...
        cfg = cls(
            ...
            dashboard=DashboardConfig(
                ...
                tailscale=_tailscale_config_from(dashboard_data.get("tailscale"), _degraded),
