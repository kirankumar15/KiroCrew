...
def is_forwarded_tailnet_request(request: web.Request, trust: TailnetTrust) -> bool:
    ...
    The public form of the RFC §2 (a)-(d) conditions, and the discriminator a
    caller needs to fail closed WITHOUT locking anyone out: under an unreadable
    identity policy a peer that could not be attributed must be denied, but a
    request that was never a forwarded tailnet request in the first place
    (loopback, the operator's own browser) resolves to no peer for the same
    reason and must be left alone. Denying on "no peer resolved" without asking
    this first would take the dashboard away from the one person who can repair
    the config.

    Synchronous and I/O-free -- the daemon is not consulted, so this is safe to
    ask inline on the event loop.
    """
    return _forwarded_peer_candidate(request, trust) is not None
...
async def governed_tailnet_trust(
    ...
    *,
    ...
    if trust.identity_unknown:
        logger.error(
            "dashboard.tailscale could not be read as configured, so the "
            "tailnet login allowlist is unknown — forwarded tailnet peers are "
            "DENIED until config.json is fixed and the gateway restarted. "
            "Local (non-forwarded) access is unaffected."
        )
    return trust
