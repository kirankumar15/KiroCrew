...
def _drop_rows_the_window_dropped(
    all_msgs: list[dict],
    snapshot: tuple[int, list[dict]],
) -> list[dict]:
    """Drop disk rows a PENDING REWRITE deleted from the window but not from disk.

    ``chat_regenerate`` truncates the window (``del slot.messages[u_idx + 1:]``),
    sets ``_pending_rewrite`` and only then tries to save. When that save fails the
    two stores disagree about the transcript's LENGTH, not merely a row's content:
    disk still holds the turns the window dropped. ``_overlay_unflushed_edits``
    cannot help there -- it substitutes by id and deliberately preserves the row
    COUNT, so the deleted turns survive the overlay and reappear on the next
    bounded refresh.

    The boundary is found by IDENTITY, never by offset. ``_disk_older_count``
    describes the current session's own file, while ``all_msgs`` is a CHAINED read
    that spans older sessions and omits transient roles, so slicing the corpus at
    that counter mixes units -- the failure ``_append_unflushed_tail`` documents at
    length -- and would cut inside real older-session history. Instead: the first
    disk row whose ``meta.mid`` the window still holds marks where the window's
    region begins. Everything above it is the frozen prefix and is never touched.

    Inside that region a row is dropped only when it CARRIES a ``mid`` the window
    does not hold. Both halves matter. Carrying an id is what makes the row a
    flushed window row (a save copies each window row's ``mid`` to disk), so an
    id-less row there is history this helper has no standing to judge and passes
    through -- the conservative direction, since a wrong drop deletes a durable
    turn. And a window that shares no id with disk yields no boundary at all, so
    nothing is dropped and the caller falls through to the existing reconciliation.
    """
    _, window = snapshot
    live_mids = {
        mid for row in window if isinstance(mid := (row.get("meta") or {}).get("mid"), str) and mid
    }
    if not live_mids:
        return all_msgs

    def _mid_of(row: dict) -> str | None:
        meta = row.get("meta")
        if not isinstance(meta, dict):
            return None
        mid = meta.get("mid")
    ...
    start = next(
        (i for i, row in enumerate(all_msgs) if _mid_of(row) in live_mids),
        None,
    )
    if start is None:
        # No shared identity, so no provable boundary: leave the corpus alone.
        return all_msgs

    kept = list(all_msgs[:start])
    for row in all_msgs[start:]:
        mid = _mid_of(row)
        if mid is not None and mid not in live_mids:
            continue
        kept.append(row)
    return kept
...
async def api_chat_slot_detail(request: web.Request) -> web.Response:
    ...
    if limit_raw is None and before_raw is None:
        ...
        # A pending rewrite means the window was TRUNCATED and the save that would
        # have shortened disk may not have landed. Settle the transcript's LENGTH
        # before reconciling its content: the two helpers below both preserve or
        # grow the corpus, so neither can retire a turn the window deleted. Read on
        # the loop beside the snapshot so the flag and the window agree.
        if getattr(slot, "_pending_rewrite", False):
            all_msgs = await asyncio.to_thread(
                _drop_rows_the_window_dropped, all_msgs, tail_snapshot
            )
        all_msgs = await asyncio.to_thread(
