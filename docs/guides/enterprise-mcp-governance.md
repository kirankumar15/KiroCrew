...
- **Every use is loud.** Pulling the lever writes a `security_policy_break_glass`
  audit record naming both tiers and logs a warning carrying the expiry, so an
  operator reading logs during an incident sees that a local document is in charge.
...
later: `kirocrew policy fetch` refuses before spending a request and says why, the
background refresh stops itself, and an install would be refused anyway as a last
guard. A local file **without** a grant does none of that — it is merely a
