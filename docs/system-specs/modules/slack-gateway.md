...
- **Config**: Enabled by default (`stt.enabled = true`). `stt.provider` selects one of six recognisers behind the single `transcribe_audio` entry point: the default `whisper` shells out to a whisper CLI, `mlx` and `parakeet` to Apple-silicon-only CLIs, `faster` runs faster-whisper inside the gateway process, `apple` drives the on-device SpeechAnalyzer helper, and `transcribe` calls AWS Transcribe. Availability therefore means "the selected provider's runtime is installed", which `transcribe.is_available()` reports; for the default that is a whisper binary on `PATH` or at `stt.whisper_path`, installed out of band (`brew install openai-whisper`, or the Speech-to-Text card's install button). Model default: `turbo` (~1.6 GB download, 809M params, ~8x faster than large), used by the Whisper-size providers. Compressed memos (ogg/Opus, webm) need a **system** FFmpeg, a separate user-installed prerequisite that `ensure_ffmpeg_in_path()` locates. Two providers need less: `faster` decodes in-process through PyAV, and `transcribe` streams an `.ogg` memo to AWS as-is (`media_encoding="ogg-opus"`), needing ffmpeg only to remux a `.webm` into ogg first. `stt.device` and `stt.timeout_secs` are configurable. Setup: [configuration](../../../src/kiro_crew/docs/configuration.md) § Speech-to-text.
