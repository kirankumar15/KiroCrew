...
- **Key + log are on the sensitive-path floor (`cdf82704`):** the `trust` directory (whole-dir entry), the legacy `sel_hmac.key` leaf, and `security_events.jsonl` are in `security._SENSITIVE_HOME_DIRS`, so the audited agent's `fs_read`/file-edit tools (gated by `is_sensitive_path()`) cannot read the key to forge the chain or rewrite the log. The gateway's own writer/reader (`sel.py`, `dashboard/session_health.py`) opens the files directly and bypasses that gate. A spawned OS subprocess reaches the key through an `open()` that never routes through `is_sensitive_path()`, so the sandbox launcher (`sandbox._build_launcher_script`) and the macOS Seatbelt profile additionally mask the `sel_hmac.key` leaf under both crew home prefixes in every tier — closing the write direction too, which is what matters for a signing key: on Linux the empty-inode bind-mount blocks reads and writes alike, and on macOS the profile carries an explicit `file-write*` deny beside the read deny so the key cannot be rewritten to re-sign a forged chain. A relocated `KIROCREW_HOME` is covered as well, via `sandbox._relocated_keystone_dirs()`. Residual: the key still lives in the agent's home namespace — a deeper out-of-process signer is future hardening.
