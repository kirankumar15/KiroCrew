 ...
 *  Carries the ISO ladder (`Sortable` in `sessionOrder.ts`) rather than
 *  `modified`, so remote rows are ranked, date-segmented and labelled by exactly
 *  the same code path as local ones. */
/** A peer slot flattened into the shape the Sessions list already renders.
 *  Deliberately satisfies `ChatSlot`'s required trio (`key`, `messages`,
 *  `running`) so a remote row can be merged into the LIVE sessions list, not just
 *  the history drawer: these are the peer's OPEN sessions, and filing live
 *  sessions under "Older Sessions" (whose empty state reads "closed tabs appear
 *  here") was a category error.
 *
